...
const setupCreateBridge = (
  props: Partial<CreateBridgeProps>
): { comp: RenderResult } => {
  const {
    isOpen = true,
    onClose = jest.fn(),
...
describe("CreateBridge component", () => {
  it("should not display anything if the modal is closed", async () => {
    const { comp } = setupCreateBridge({ isOpen: false });
  ...
  it("should ask for instance name before creating an instance", async () => {
    ...
    const { comp } = setupCreateBridge({ createBridge });
    await waitForI18n(comp);
    await waitFor(() => {
      expect(
  ...
  it("should retrieve the schema of the selected handling strategy, when changing the method", async () => {
    ...
    const { comp } = setupCreateBridge({ getSchema });
    await waitForI18n(comp);
  ...
  it("should be dismissed when canceling", async () => {
    ...
    const { comp } = setupCreateBridge({ onClose });
    await waitForI18n(comp);
